...
setwd("C:/Users/t.cashion/Google Drive/Desktop/SESYNC/SensitivityAnalysisOutputs")
...
write_csv(group_data, "../DataOutputs/SA_AnnualAverageData_tidy.csv")
group_data <- read_csv("../DataOutputs/SA_AnnualAverageData_tidy.csv")
...
for (folder in folders) {
  ...
  for (file in files){
    ...
    df_design <- gsub(df_design, pattern="_\\d\\d", replacement="")
    df_design <- gsub(df_design, pattern="_\\d", replacement="")
    df_design <- gsub(df_design, pattern="_dispersal.*", replacement="")
    
    df_scenario <- gsub(df_name, pattern="/.*", replacement="")
    df_scenario <- gsub(df_scenario, pattern=paste(df_RCP, "_", df_design, "_",  sep=""), replacement="")
    df_scenario <- gsub(df_scenario, pattern="_dispersal.*", replacement="")
    ...
    df_dispersal <- folder
    df_dispersal <- gsub(df_dispersal, pattern=".*_dispersal", replacement="")
...
write_csv(group_data, "../DataOutputs/SA_SpatialData_tidy.csv")
#write_csv(group_data %>% filter(scenario=="10"), "../DataOutputs/SA_SpatialData_tidy_10.csv")
