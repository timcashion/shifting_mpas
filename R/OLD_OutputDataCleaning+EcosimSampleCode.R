...
fleet_data_catch %>% 
  ...
  geom_line() +
  facet_wrap(~RCP+scenario, nrow=2) +
  theme_classic() + 
  ylab("Catch") + 
  NULL


group_biomass <- group_data %>% 
...
spatial_biomass_anchovy <- group_data %>% 
  filter(type=="Biomass") %>%
  filter(fg=="Anchovy") %>% 
  filter(year==100) %>% 
  filter(scenario=="MPA_Square_Shifting") %>% 
  ggplot(aes(x=col_num, y=rev(row_num), fill=value)) + 
  geom_tile() +
  scale_fill_gradientn(colours = pal)  + 
  scale_x_continuous(expand=c(0,0)) + 
  scale_y_continuous(expand=c(0,0)) + 
  NULL 
print(spatial_biomass_anchovy)
...
spatial_biomass_whiting <- group_data %>% 
  filter(type=="Biomass") %>%
  filter(fg=="Whiting") %>% 
  filter(year==100) %>% 
  ggplot(aes(x=col_num, y=rev(row_num), fill=value)) + 
...
input_files <- list.files("../InputData/")
input_files <- input_files[grepl(input_files, pattern="Anchovy Bay-MPA")]
file <- input_files[1]
mpas <- tibble()

for (file in input_files){
  scenario_name <- gsub(file, pattern="Anchovy Bay-MPA - ", replacement="")
  scenario_name <- gsub(scenario_name, pattern=".csv", replacement="")
  mpa <- read_csv(paste("../InputData/", file, sep=""))
  ...
  mpa_melt$scenario <- paste("MPA_", scenario_name, sep="")
  mpa_melt$col_num <- as.numeric(mpa_melt$col_num)
  mpa_melt$mpa <- as.factor(as.character(mpa_melt$mpa))
  year_frame <- data_frame(year=seq(0,100,1), scenario=paste("MPA_", scenario_name, sep=""))
  mpa_frame <- left_join(year_frame, mpa_melt)
  mpas <- bind_rows(mpas, mpa_frame)
}

mpas$scenario <- gsub(mpas$scenario, pattern="-4", replacement="")
...
temp$mpa[which(grepl(temp$scenario, pattern="MPA_Square_Shifting") & temp$mpa==1 & temp$m1==1)] <- 1 
temp$mpa[which(grepl(temp$scenario, pattern="MPA_Square_Shifting") & temp$mpa==1 & temp$m1==0)] <- 0 
temp$mpa[which(grepl(temp$scenario, pattern="MPA_Square_Shifting") & temp$mpa==2 & temp$m1==1)] <- 2 
temp$mpa[which(grepl(temp$scenario, pattern="MPA_Square_Shifting") & temp$mpa==2 & temp$m1==0)] <- 0 

#temp$scenario <- gsub(temp$scenario, pattern=" - 4", replacement="")


temp <- temp %>% select(-m1)

mpas <- temp
mpas$scenario <- gsub(mpas$scenario, pattern=" -..", replacement="")
mpas <- mpas %>% 
  mutate(mpa= as.numeric(mpa)) %>% 
  group_by(year, scenario, row_num, col_num) %>% 
  summarize(mpa = sum(mpa))

write_csv(mpas, "../DataOutputs/MPAData_tidy.csv")
mpas <- read_csv("../DataOutputs/MPAData_tidy.csv")
####Add in MPA outlines to group data maps####
mpa_spatial <- left_join(group_data, mpas)
mpa_spatial$mpa[which(mpa_spatial$scenario=="NoMPA")] <- 0
x <- mpa_spatial[is.na(mpa_spatial$mpa),] 
#mpa_spatial$mpa[is.na(mpa_spatial$mpa)] <- 0 
#mpas <- mpas %>% filter(mpa=="1")
colors <- c(NA, "red", "blue")
mpa_spatial$mpa <- as.factor(mpa_spatial$mpa)
spatial_biomass_whiting <- mpa_spatial %>% 
  filter(type=="Biomass") %>%
  filter(fg=="Whiting") %>% 
  filter(year==100) %>% 
  ggplot(aes(x=col_num, y=rev(row_num), fill=value, color=mpa)) + 
  geom_tile() +
  scale_fill_gradientn(colours = pal)  + 
  ...
  xlab("") +
  ylab("") + 
  labs(fill="Biomass (t/km2)", color="MPA") + 
  #geom_tile(data=mpa_spatial, aes(x=col_num, y=rev(row_num), color=mpa)) +
  #scale_colour_manual(breaks = mpa, values = c(0 = NA, 1 = 'black')) +
  facet_wrap(~RCP+scenario, nrow=2) +
  NULL 
print(spatial_biomass_whiting)
ggsave("../VisualOutputs/Spatial Biomass of Whiting End of Century.png", width=12, height=8, units=c("in"), dpi=1000)
...
df <- rbind(output_table_biomass, output_table_catch, output_table_effort)

df <- df %>% ungroup() 

write_csv(df, "../DataOutputs/Output Tables for ScenarioSummaries.csv")
